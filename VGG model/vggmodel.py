...
def create_vgg_model(drop_out=False,batch_normalization=False,weight_decay=False,activation='relu',optimizer='adam'):
	...
	model.add(Conv2D(32, (3, 3), strides = 1, padding = "same", activation = activation, input_shape = (32, 32, 3)))
	if weight_decay == True:
	...
	model.add(Conv2D(32, (3, 3), strides = 1, padding = "same", activation = activation))
	if weight_decay == True:
		model.add(ActivityRegularization(l2=0.005))
	...
	model.add(Conv2D(64, (3, 3), strides = 1, padding = "same", activation = activation))
	if weight_decay == True:
		model.add(ActivityRegularization(l2=0.005))
	if batch_normalization == True:
		model.add(BatchNormalization())
