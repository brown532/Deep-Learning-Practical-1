...
def standard_CNN_Model(drop_out=False,batch_normalization=False,weight_decay=False,activation='relu',optimizer='adam'):
    ...
    #add model layers
    # model = Sequential()
    model.add(Conv2D(64, (3,3), activation=activation,padding='same', input_shape=(32,32,3), kernel_regularizer = weight_decay))
    if batch_normalization == True:
        model.add(BatchNormalization())

    ##################################

    model.add(Conv2D(32, (3,3), padding='same', activation=activation,kernel_regularizer = weight_decay))
    if batch_normalization == True:
        model.add(BatchNormalization())
